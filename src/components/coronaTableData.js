...
export default function ({ stateDistrictWiseData, statePatients, state }) {
  ...
  Object.keys(districts).forEach((key) => {
    let item = createData(
      i18n.t(key),
      districts[key].confirmed,
      districts[key]["delta"].confirmed,
      key
    );
    tableData.push(item);
  });

  tableData = tableData.sort(function (l, r) {
    return r.confirmed - l.confirmed;
  });
  ...
  function getCityPatients(district, city) {
    let districtDetails = statePatients.filter(
    ...
    if (city === "Today") {
      result = districtDetails.filter(
        (patient) => patient.dateannounced === todayDate
      );
    } else if (city !== "Unknown") {
      result = districtDetails.filter(
        (patient) => patient.detectedcity === city
      );
    } else {
